from collections import defaultdict

import frappe
from frappe import _
from frappe.utils import flt, getdate

from marina_custom_apps.stock_transfer_audit.control_service import (
    get_receive_owner_maps,
    get_variance_value_maps,
)


def execute(filters=None):
    filters = frappe._dict(filters or {})
    if not filters.from_date or not filters.to_date:
        frappe.throw(_("From Date and To Date are required."))
    return get_columns(), get_data(filters)


def get_columns():
    return [
        {"label": _("Receiver Username"), "fieldname": "receiver_username", "fieldtype": "Data", "width": 145},
        {"label": _("Target Warehouse"), "fieldname": "target_warehouse", "fieldtype": "Link", "options": "Warehouse", "width": 180},
        {"label": _("Transfers Received"), "fieldname": "transfer_count", "fieldtype": "Int", "width": 105},
        {"label": _("Variance Transfers"), "fieldname": "variance_count", "fieldtype": "Int", "width": 105},
        {"label": _("Clean %"), "fieldname": "clean_pct", "fieldtype": "Percent", "width": 80},
        {"label": _("Qty Sent"), "fieldname": "qty_sent", "fieldtype": "Float", "precision": 3, "width": 85},
        {"label": _("ABS Variance"), "fieldname": "abs_variance", "fieldtype": "Float", "precision": 3, "width": 95},
        {"label": _("Variance %"), "fieldname": "variance_pct", "fieldtype": "Percent", "width": 85},
        {"label": _("Variance Value"), "fieldname": "variance_value", "fieldtype": "Currency", "width": 110},
        {"label": _("Ignored Qty"), "fieldname": "ignored_qty", "fieldtype": "Float", "precision": 3, "width": 85},
        {"label": _("Ignored Count"), "fieldname": "ignored_count", "fieldtype": "Int", "width": 90},
    ]


def get_data(filters):
    records = frappe.get_all(
        "Stock Transfer Audit Record",
        filters={"docstatus": 1},
        fields=[
            "name",
            "receive_stock",
            "target_warehouse",
            "audit_result",
            "total_sent_qty",
            "total_abs_variance_qty",
        ],
        limit_page_length=0,
    )
    if not records:
        return []

    receive_names = [r.receive_stock for r in records if r.receive_stock]
    receive_rows = frappe.get_all(
        "Stock Entry",
        filters={"name": ["in", receive_names]},
        fields=["name", "posting_date"],
        limit_page_length=0,
    ) if receive_names else []
    dates = {r.name: getdate(r.posting_date) for r in receive_rows}
    _, usernames = get_receive_owner_maps(receive_names)

    selected = []
    for row in records:
        if not row.receive_stock or row.receive_stock not in dates:
            continue
        if not (getdate(filters.from_date) <= dates[row.receive_stock] <= getdate(filters.to_date)):
            continue
        if filters.target_warehouse and row.target_warehouse != filters.target_warehouse:
            continue
        receiver = usernames.get(row.receive_stock, "")
        if filters.receiver_username and receiver != filters.receiver_username:
            continue
        row.receiver_username = receiver
        selected.append(row)

    if not selected:
        return []

    names = [r.name for r in selected]
    values, _ = get_variance_value_maps(names)

    ignored_qty = defaultdict(float)
    ignored_count = defaultdict(int)
    for row in frappe.get_all(
        "Stock Transfer Audit Item",
        filters={
            "parent": ["in", names],
            "parenttype": "Stock Transfer Audit Record",
            "action": "Ignore",
        },
        fields=["parent", "discrepancy_qty"],
        limit_page_length=0,
    ):
        ignored_qty[row.parent] += abs(flt(row.discrepancy_qty))
        ignored_count[row.parent] += 1

    grouped = {}
    for row in selected:
        key = (row.receiver_username or _("Unknown"), row.target_warehouse or "")
        g = grouped.setdefault(
            key,
            {
                "receiver_username": row.receiver_username or _("Unknown"),
                "target_warehouse": row.target_warehouse,
                "transfer_count": 0,
                "variance_count": 0,
                "qty_sent": 0.0,
                "abs_variance": 0.0,
                "variance_value": 0.0,
                "ignored_qty": 0.0,
                "ignored_count": 0,
            },
        )
        g["transfer_count"] += 1
        g["variance_count"] += 1 if row.audit_result == "Variance" else 0
        g["qty_sent"] += flt(row.total_sent_qty)
        g["abs_variance"] += flt(row.total_abs_variance_qty)
        g["variance_value"] += flt(values.get(row.name))
        g["ignored_qty"] += flt(ignored_qty.get(row.name))
        g["ignored_count"] += ignored_count.get(row.name, 0)

    result = []
    for g in grouped.values():
        g["clean_pct"] = (
            ((g["transfer_count"] - g["variance_count"]) / g["transfer_count"]) * 100
            if g["transfer_count"] else 0
        )
        g["variance_pct"] = (
            (g["abs_variance"] / g["qty_sent"]) * 100 if g["qty_sent"] else 0
        )
        result.append(g)

    result.sort(
        key=lambda r: (
            -r["variance_pct"],
            -r["variance_value"],
            r["receiver_username"],
        )
    )
    return result
