...
def _submitted_receive_for_send(send_name):
    rows = frappe.get_all("Stock Entry", filters={"outgoing_stock_entry": send_name, "stock_entry_type": RECEIVE_TYPE, "docstatus": 1}, fields=["name"], order_by="posting_date desc, creation desc", limit_page_length=1)
...
def get_transfer_snapshot(send_name, receive_name=None):
    ...
    if receive:
        for row in receive.items or []:
            ...
            items.append({"item_code":row.item_code,"sent_qty":sent,"actual_received_qty":actual,"discrepancy_qty":variance,"unexpected_item":0,"send_stock_detail":row.get("custom_original_send_stock_detail") or row.get("ste_detail"),"receive_stock_detail":row.name})
        ...
        for row in receive.get("custom_unexpected_received_items") or []:
            ...
            items.append({"item_code":row.get("item_code"),"sent_qty":0,"actual_received_qty":actual,"discrepancy_qty":variance,"unexpected_item":1,"send_stock_detail":None,"receive_stock_detail":None})
            total_received += actual; total_abs_variance += abs(variance)
    else:
        for row in send.items or []:
            sent=flt(row.qty); total_sent += sent
            items.append({"item_code":row.item_code,"sent_qty":sent,"actual_received_qty":0,"discrepancy_qty":sent,"unexpected_item":0,"send_stock_detail":row.name,"receive_stock_detail":None})
    total_variance=total_sent-total_received
    return {"original_send_stock":send.name,"receive_stock":receive.name if receive else None,"posting_date":receive.posting_date if receive else send.posting_date,"source_warehouse":send.from_warehouse,"transit_warehouse":send.to_warehouse,"target_warehouse":receive.to_warehouse if receive else send.get("custom_intended_final_warehouse"),"total_sent_qty":total_sent,"total_received_qty":total_received,"total_variance_qty":total_variance,"total_abs_variance_qty":total_abs_variance,"audit_result":"Clean" if total_abs_variance == 0 else "Variance","items":items}
...
def get_unaudited_received_transfers(from_date, to_date):
    ...
    receives=frappe.get_all("Stock Entry", filters={"docstatus":1,"stock_entry_type":RECEIVE_TYPE,"posting_date":["between",[from_date,to_date]],"outgoing_stock_entry":["is","set"]}, fields=["name","outgoing_stock_entry","posting_date","posting_time","creation"], order_by="posting_date asc, posting_time asc, creation asc", limit_page_length=0)
